...
\ifthenelse{\boolean{ifWeb}}{
        \begin{tabbing}
          ...
          \> {\small \texttt{~~\pdfltsub}~$\uparrow$~~~~ \phantom{}~} \> {  }  \pdflttxt \\ 
  ...
  \begin{small}
    \begin{tabular}[c]{lll}
      ...
      \htmlvers && \htmltext \\
      \githrepo && \githtext \\
      ~~\pdfltsub~$\uparrow$ && \pdflttxt \\
      ~~\slidesub\phantom{~$\uparrow$} && \slidetxt \\
      ~~\bibtlink\phantom{~$\uparrow$}  && \bibttext  
    \end{tabular}
  \end{small}
}
...
\begin{abstract}
  ...
  First, during lockdowns, many types of spending are undesirable or impossible.
  Second, some of the jobs that disappear during the lockdown will not reappear.
  We estimate that, if the lockdown is short-lived (the median point of view in April 2020), the combination of expanded unemployment insurance benefits and stimulus payments should be sufficient to allow a swift recovery in consumer spending to pre-crisis levels.  If the lockdown lasts longer (or there is a `second wave'), an extension of \href{https://www.dol.gov/coronavirus/unemployment-insurance}{enhanced unemployment benefits} will likely be necessary for consumption spending to recover quickly.
\end{abstract}
